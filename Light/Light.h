...
class Light {
private:
    Servo Botton_Servo; //云台底座的舵机
    Servo Head_Servo;   //上面的舵机
...
public:
    ...
    Light(uint8_t pina,uint8_t pinb);
    ~Light();
...
Light::Light(uint8_t pina,uint8_t pinb ){
    Botton_Servo.attach(pina);
    Head_Servo.attach(pinb);
}
...
void Light ::  Turn(){
    Botton_Servo.write( 180 - Sigma);
    Head_Servo.write( 180 - Phi );
}

void Light :: Turn(float phi, float sigma){
    Phi = phi ;
    Sigma = sigma ;
